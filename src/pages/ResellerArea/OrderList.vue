			...
			<q-page class="bg-white">
					...
					<div class="row q-px-lg q-py-md">
						<div class="col">
							<div
								...
								:class="orderStatus === '' ? 'text-amber-9' : ''"
							>
								<q-icon
						...
						<div class="col">
							<div
								@click="changeStatus('waiting_payment')"
								style="cursor:pointer"
								:class="orderStatus === 'waiting_payment' ? 'text-amber-9' : ''"
							>
								<q-icon
									name="fas fa-wallet"
									style="font-size: 30px; margin: 0 0 5px 0"
						...
						<div class="col">
							<div
								...
								style="cursor:pointer"
								:class="orderStatus === 'processed' ? 'text-amber-9' : ''"
							>
								<q-icon
						...
						<div class="col">
							<div
								...
								style="cursor:pointer"
								:class="orderStatus === 'shipped' ? 'text-amber-9' : ''"
							>
								<q-icon
						...
						<div class="col">
							<div
								...
								style="cursor:pointer"
								:class="orderStatus === 'pending' ? 'text-amber-9' : ''"
							>
								<q-icon
									name="fas fa-clock"
									style="font-size: 30px; margin: 0 0 5px 0"
						...
						<div class="col q-px-md">
							<template v-if="orderData.length !== 0">
								...
								>
									<div class="row">
										<div class="col-xs-8">
											<h6
												v-if="order.detail_customer.length !== 0"
												style="margin: 0; font-size: 12px; font-family: 'Open Sans'; line-height: 16px"
											>
												...
												Nama Pelanggan :
												{{ order.detail_customer[0].customer_name }}
											</h6>
										</div>
										<div class="col-xs-4">
											<router-link
											...
											>
												<p
													...
													class="text-orange-8 text-right"
												>
													Lihat Detail
												</p>
											</router-link>
									...
									<div class="row">
										<div class="col-xs-6">
											<h6
												style="margin: 0; font-size: 12px; font-family: 'Open Sans'; line-height: 16px"
											>
												Total Pembayaran<br /><b
													>Rp{{ formatPrice(order.grand_total) }}</b
												>
											</h6>
										</div>
										<div class="col-xs-6">
											...
											>
												...
												<span
													style="font-weight: bold;"
													class="text-primary"
													v-else-if="order.status === 'payment_confirmed'"
													>Sudah Dibayar</span
												>
												<span
													style="font-weight: bold;"
													class="text-yellow-10"
													v-else-if="order.status === 'processed'"
							...
							<template v-else>
								<div class="q-pa-md" style="padding-top: 150px">
									<center>
										<img
											src="~/assets/images/components/box_order.png"
											width="100"
										/>
              ...
              <div class="row q-pa-md">
				...
				<q-page-sticky position="bottom" :offset="[18, 18]">
					<q-btn
						@click="filterDateDialog = true"
						rounded
						color="amber-8 text-capitalize"
						><q-icon name="filter_list" size="xs" class="q-mr-sm" />
						Filter</q-btn
					>
				</q-page-sticky>
			</q-page>
	...
	methods: {
		getOrder() {
			...
			if (
				...
				// 8. Filter Tanggal & Search dari Status Tertentu
				formatUrl =
					getOrderUrl +
					"/" +
					this.user.id +
					...
					"&searchInvoice=" +
					this.searchInvoice;
			} else if (this.searchInvoice !== "" && this.orderStatus !== "") {
				// 7. Filter Search dari  Status Tertentu
				formatUrl =
					getOrderUrl +
					"/" +
					this.user.id +
					...
					"&searchInvoice=" +
					this.searchInvoice;
			} else if (this.dateFilter !== "" && this.orderStatus !== "") {
				// 6. Filter Tanggal dari Status tertentu v
				formatUrl =
					getOrderUrl +
					"/" +
				...
				// 5. Filter berdasarkan Status tertentu v
				formatUrl =
					getOrderUrl + "/" + this.user.id + "?orderStatus=" + this.orderStatus;
			} else if (this.dateFilter !== "" && this.searchInvoice !== "") {
				// 4.Filter Tanggal & Search dari Semua Pesanan v
				formatUrl =
					getOrderUrl +
					"/" +
				...
				// 2.Filter Tanggal dari Semua Pesanan v
				formatUrl =
					getOrderUrl + "/" + this.user.id + "?dateFilter=" + this.dateFilter;
			} else {
				// 1.Lihat Semua Pesanan v
				formatUrl = getOrderUrl + "/" + this.user.id;
			}
			...
			axios
				.get(formatUrl, { headers: getHeader() })
				.then(response => {
					console.log(response);
						...
						for (let i = 0; i < response.data.data.length; i++) {
							...
							axios
								.get(
									showCustomerUrl +
										"/" +
										response.data.data[i].customer_address_id,
									{ headers: getHeader() }
								)
								.then(response => {
