                  ...
                  <span class="text-green">{{
                     'Rp' +
                        formatPrice(
                           Number(
                              selectedVariant.price -
                                 selectedVariant.reseller_pro_price
                           ) * Number(this.qty)
                        )
                  }}</span>
               </h4>
               ...
               >
                  ...
                  <span class="text-green">{{
                     `Rp${formatPrice(
                        Number(
                           selectedVariant.price -
                              selectedVariant.reseller_free_price
                        ) * Number(this.qty)
                     )}`
                  }}</span>
               </h4>
            ...
            <div class="row q-px-md">
               <div class="col" v-if="!isEmpty(productData)">
                  ...
                  <div class="row">
                     ...
                     >
                        ...
                        >
                           Rp{{
                              formatPrice(
                                 selectedVariant.reseller_exclusive_price * qty
                              )
                           }}
                        </h5>
                     </div>
                     ...
                     >
                        ...
                        >
                           Rp{{
                              formatPrice(
                                 selectedVariant.reseller_pro_price * qty
                              )
                           }}
                        </h5>
                     </div>
                     ...
                     >
                        ...
                        >
                           Rp{{
                              formatPrice(selectedVariant.reseller_free_price)
                           }}
                        </h5>
                     </div>
                  </div>
                  ...
                  <div
                     class="row"
                     v-if="
                        globalState.userProfile.role.id === 8 && selectedVariant
                     "
                  >
                     <div class="col">
                     ...
                     >
                        <thead>
                           <tr>
                              <th
                                 v-for="(header,
                                 i) in yaumeeSpreadsheetsTable.headers"
                                 :key="i"
                              >
                                 {{ header }}
                              </th>
                        ...
                        <tbody>
                           <tr
                              v-for="(row, i) in yaumeeSpreadsheetsTable.rows"
                              :key="i"
                           >
                              <td>{{ row.Kode }}</td>
                              <td>{{ row.Warna }}</td>
               ...
               <q-card-section>
                  ...
                  <div class="row">
                     <div class="col text-center">
                        <h5
                           style="font-size: 18px; margin: 0; line-height: 24px"
                        >
                  ...
                  <div class="row">
                     <div class="col">
                        ...
                        <h5
                           style="font-size: 10px; margin: 0; text-align: center;"
                        >
                           Atau
                        </h5>
...
export default {
   ...
   computed: {
      ...
      stockReady: function() {
         if (this.selectedVariant)
            return (
               this.selectedVariant.stock_qty -
               this.selectedVariant.keep_stock_qty
            );
         return null;
      },
   ...
   async created() {
      ...
      let sheet;
      sheet = await doc.sheetsByIndex[6];
      ...
      async getInputOptions() {
         ...
         optionRes.data.data.forEach((option) => {
            _inputOptions.push({
               ...
               option_value: [
                  ...new Set(
                     option.sku_values.map((sku_value) => sku_value.value)
                  ),
               ],
            });
         });
         ...
         if (allSku.length === 1) {
            ...
            for (let index = 0; index < allSku[0].length; index++) {
               allSku.forEach((skuIDArr) => {
                  isAvailable.push(
                     skuIDArr.some((skuID) => skuID == allSku[0][index])
                  );
