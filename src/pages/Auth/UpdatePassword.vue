  ...
  <q-layout view="lHh Lpr lFf" class="mobile-layout-on-desktop">
    ...
    <q-header class="mobile-layout-on-desktop">
      <q-toolbar class="bg-distrodakwah text-white">
        ...
        <q-toolbar-title><span style="font-size: 16px; font-weight: bold">Ganti Password</span></q-toolbar-title>
      </q-toolbar>
    </q-header>
...
export default {
  ...
  methods: {
    async onUpdate() {
      ...
      updatePasswordForm.set("newPassword_confirmation",this.newPasswordConfirmation);

      try {
        let response = await this.$axios.post(apiDomain + "/auth/updatePassword", updatePasswordForm, { headers: getHeader() }        );
